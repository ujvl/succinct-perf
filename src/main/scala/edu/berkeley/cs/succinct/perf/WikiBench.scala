...
object WikiBench {
  ...
  def extractRDD(rdd: RDD[Array[Byte]], partitionOffsets: Seq[Long], partitionSizes: Seq[Long],
    ...
    if (extractResults.length != 1) {
      throw new ArrayIndexOutOfBoundsException("Invalid output; size = " + extractResults.length
  ...
  def benchRegexSparkRDD(rdd: RDD[Array[Byte]]): Unit = {
    ...
    regex.foreach(r => {
      ...
      for (i <- 1 to numRepeats) {
        val startTime = System.currentTimeMillis()
        val results = regexSearchRDD(rdd, partitionOffsets, r)
        count += results.count()
        val endTime = System.currentTimeMillis()
        val totTime = endTime - startTime
        time += totTime
